...
export async function recalcularProgressoObra(obraId: string, companyId: string): Promise<void> {
    ...
    if (todasConcluidas && progresso >= 100) {
        status = "Concluida";
    } else if (obra.endDate < today && progresso < 100 && status !== "Pausada" && status !== "Cancelada") {
        await prisma.obra.update({ where: { id: obraId }, data: { status: "Atrasada" } });
    } else if (status === "Atrasada" && obra.endDate >= today) {
        status = "Em andamento";
    }
