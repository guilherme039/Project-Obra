...
export async function gerarAlertas(obraId: string, companyId: string): Promise<Alerta[]> {
    ...
    if (obra && obra.dataPrevisaoTermino && obra.dataPrevisaoTermino < today && obra.progresso < 100 && obra.status !== "Cancelada" && obra.status !== "Pausada") {
        ...
        if (diasAtraso > 0) {
            alertas.push({
                tipo: "atraso",
                titulo: `Obra Atrasada (${diasAtraso} dias)`,
